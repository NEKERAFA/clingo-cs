...
using ClingoSolveResult = ClingoSharp.CoreServices.Enums.SolveResult;
...
namespace ClingoSharp
{
    ...
    public sealed class SolveHandle : IEnumerable<Model>
    {
        ...
        public SolveResult Get()
        {
            ...
            if (result.HasFlag(ClingoSolveResult.Satisfiable))
            {
                satisfiable = true;
            } 
            else if (result.HasFlag(ClingoSolveResult.Unsatisfiable))
            {
                satisfiable = false;
            }
            ...
            return new SolveResult()
            {
                ...
                IsExhausted = result.HasFlag(ClingoSolveResult.Exhausted),
                IsInterrupted = result.HasFlag(ClingoSolveResult.Interrupted)
            };
        }
